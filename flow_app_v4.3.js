 ...
 * 
 * VERSION: v4.3.0
 * STATUS: Development Ready - Flexible Period System
 * DATE: September 6, 2025
 * LAST UPDATED: September 6, 2025 - Flexible Period Implementation
 * 
 * ====================================================================
 * RECENT CHANGES (v4.3.0):
 * ====================================================================
 * 
 * 🎯 FLEXIBLE PERIOD SYSTEM:
 * - Added support for weekly, bi-weekly, and monthly periods
 * - Rolling period reset logic with custom start dates
 * - Period-based income calculations and daily flow adjustments
 * - v4.3 data namespace for safe parallel deployment
 * 
 * 🔧 TECHNICAL IMPROVEMENTS:
 * - Enhanced data migration system for existing v4.2 users
 * - Period configuration storage and validation
 * - Updated Flow Method allocations (60% Foundation, 25% Future, 15% Freedom)
 * - Dynamic daily flow calculation based on period length
 * 
 * 📱 GIGI ECONOMY FOCUS:
 * - Designed for modern earning patterns (gig work, freelance, part-time)
 * - Flexible period alignment with actual income schedules
 * - Custom start date selection for payday alignment
 * - Rolling period system for consistent financial management
 * 
 * ====================================================================
 * PREVIOUS VERSIONS:
 * ====================================================================
...
const appState = {
    // ===== V4.3: VERSION AND DATA NAMESPACE =====
    version: '4.3',
    ...
    periodConfig: {
        ...
        startDate: null,              // User selected start date (YYYY-MM-DD)
...
// ===== v4.3 ENHANCED DAILY FLOW CALCULATION WITH PERIOD SUPPORT =====
function calculateDailyFlow(categories, currentDay, daysInMonth) {
    FlowAppLogger.debug('v4.3 Daily Flow Calculation Started', {
        categoriesProvided: !!categories,
        currentDay,
        daysInMonth,
        periodConfigAvailable: !!(appState?.periodConfig?.length)
    });
    ...
    let daysRemaining, rawDailyFlow, calculationMethod;

    // v4.3 ENHANCEMENT: Check if period configuration is available and should be used
    if (appState?.periodConfig?.length && appState.periodConfig.length > 0) {
        ...
        // Calculate days remaining in current period cycle
        if (appState.periodConfig.startDate) {
            // Parse the start date string safely in local timezone to avoid UTC shifts
            const startDateString = appState.periodConfig.startDate;
            ...
            startDate.setHours(0, 0, 0, 0); // Reset to start of day (already local)
            
            const daysDiff = Math.floor((now - startDate) / (1000 * 60 * 60 * 24));
            const periodDaysElapsed = daysDiff % periodLength; // Days elapsed in current period cycle
            // Days remaining includes today, so it's periodLength - periodDaysElapsed
            const periodDaysRemaining = Math.max(periodLength - periodDaysElapsed, 1);
            daysRemaining = periodDaysRemaining;
        } else {
            // Fallback: assume we're at the beginning of a period
            daysRemaining = periodLength;
        }
        
        rawDailyFlow = remaining / daysRemaining;
        calculationMethod = 'period-based';
        
        FlowAppLogger.debug('Using period-based calculation', {
            periodType: appState.periodConfig.type,
            periodLength,
            daysElapsed: appState.periodConfig.startDate ? 
                Math.floor((new Date() - new Date(appState.periodConfig.startDate)) / (1000 * 60 * 60 * 24)) % periodLength : 0,
            daysRemaining
        });
    } else {
        // Fall back to monthly calculation for backward compatibility
        const validDaysInMonth = typeof daysInMonth === 'number' ?
            daysInMonth :
            new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate();
        
        daysRemaining = validDaysInMonth - validCurrentDay + 1;
        rawDailyFlow = remaining / daysRemaining;
        calculationMethod = 'monthly-based';
        
        FlowAppLogger.debug('Using monthly-based calculation (fallback)', {
    ...
    FlowAppLogger.debug('v4.3 Daily Flow Calculation Complete', {
        income,
        spent,
        remaining,
...
const CrossSystemValidator = {
    ...
    validateMathematicalAccuracy() {
        try {
            ...
            const roundedDailyFlow = Math.round(rawDailyFlow); // $1 precision
...
function processTransaction(amount, description, category = 'freedom') {
    ...
    if (category === 'freedom') {
        ...
        const dailyFlowAmount = calculateDailyFlow(appState.categories); // Use enhanced function
...
function updateDailyFlowDisplay() {
    // **DUAL DISPLAY SYSTEM: Use dailyFlowFixed minus todayFlowed (same as updateAllDisplaysSynchronized)**
    const dailyFlow = calculateDailyFlow(appState.categories);
    const todayFlowed = appState.todayFlowed || 0;
    const dailyFlowFixed = appState.dailyFlowFixed || dailyFlow; // Fallback to calculated if not set
    const remainingToday = dailyFlowFixed - todayFlowed; // ALLOW NEGATIVE VALUES
...
function updateAllDisplaysSynchronized() {
    ...
    // **DUAL DISPLAY SYSTEM: Show dailyFlowFixed minus todayFlowed (ALLOW NEGATIVE)**
    const todayFlowed = appState.todayFlowed || 0;
    const dailyFlowFixed = appState.dailyFlowFixed || dailyFlow; // Fallback to calculated if not set
    const remainingToday = dailyFlowFixed - todayFlowed; // ALLOW NEGATIVE VALUES
...
function saveToLocalStorage() {
    try {
        const dataToSave = {
            ...
            // V4.3 Period Configuration
            periodConfig: {
                type: appState.periodConfig?.type || 'monthly',
                length: appState.periodConfig?.length || 30,
                startDate: appState.periodConfig?.startDate || new Date().toISOString().split('T')[0],
                incomeAmount: appState.periodConfig?.incomeAmount || appState.monthlyIncome || 3200,
                lastReset: appState.periodConfig?.lastReset,
                nextReset: appState.periodConfig?.nextReset
            },
            ...
            budgetState: {
                ...
                currentPeriod: appState.currentPeriod || new Date().toISOString().slice(0, 7)
            },
            ...
            appSettings: {
                version: "4.3.0-flexible-periods",
                dataVersion: 4.3,
                backupTimestamp: Date.now()
            }
        };

        // Save to v4.3 namespace
        localStorage.setItem('flowAppData_v4_3', JSON.stringify(dataToSave));
...
function loadFromLocalStorage() {
    try {
        // V4.3: Load with migration support
        const savedData = loadV43DataWithMigration();
        ...
        // CRITICAL: Check if this is fresh onboarding data from v4.3 onboarding
        if (savedData.userProfile?.onboardingCompleted && !savedData.achievements?.welcomeShown) {
            return savedData;
        }

        // Check for legacy incomplete data that should be replaced
        if (!savedData.userProfile?.setupCompleted && !savedData.onboardingComplete) {
            return null;
        }

        // Check if this is a fresh onboarding completion
        if (savedData.achievements?.onboardingComplete && !savedData.achievements?.welcomeShown) {
            showToast(`🎉 Welcome to Flow! Your $${savedData.budgetState.dailyFlow} daily flow is ready!`, 'success');

            // Mark welcome as shown
            savedData.achievements.welcomeShown = true;
            localStorage.setItem('flowAppData_v4_3', JSON.stringify(savedData));
        }

        // Validate data structure
        if (validateLoadedData(savedData)) {
            FlowAppLogger.info('Valid v4.3 data loaded successfully', {
                version: savedData.version || '4.3',
                income: savedData.userProfile?.monthlyIncome,
                periodType: savedData.periodConfig?.type,
                transactionCount: savedData.transactions?.length || 0,
                lastSaved: new Date(savedData.userProfile?.lastUpdated).toLocaleString()
            });
            return savedData;
        } else {
            ...
            if (minimalData) {
                ...
                return {
                    ...
                    periodConfig: {
                        ...
                        incomeAmount: parsedMinimal.monthlyIncome || 3200,
                        lastReset: null,
                        nextReset: null
                    },
                    budgetState: { categories: {}, dailyFlow: 0 },
                    transactions: [],
                    appSettings: { version: "4.3.0-minimal-fallback", dataVersion: 4.3 }
                };
            }
...
function initializeWithPersistentData() {
    ...
    if (savedData) {
        // V4.3: Handle period system initialization
        if (savedData.version === '4.3') {
            return processV43Data(savedData);
        }
...
function processV43Data(savedData) {
    try {
        console.log('🔄 Processing v4.3 data with period system...');
        
        // Initialize period configuration
        initializePeriodConfig(savedData);
        
        // Check for period reset
        checkAndHandlePeriodReset();
        
        // Restore income from period config or user profile
        if (savedData.periodConfig?.incomeAmount) {
            appState.monthlyIncome = savedData.periodConfig.incomeAmount;
        } else if (savedData.userProfile?.monthlyIncome) {
            appState.monthlyIncome = savedData.userProfile.monthlyIncome;
        }

        // Set onboarding as complete
        appState.onboardingComplete = savedData.userProfile?.onboardingCompleted || true;

        // Handle v4.3 categories with updated Flow Method allocations
        if (savedData.budgetState?.categories || savedData.categories) {
            const cats = savedData.budgetState?.categories || savedData.categories;
            appState.categories = {
                foundation: {
                    allocated: cats.foundation?.allocated || Math.round(appState.monthlyIncome * 0.60),
                    used: cats.foundation?.used || 0,
                    percentage: cats.foundation?.percentage || 60
                },
                future: {
                    allocated: cats.future?.allocated || Math.round(appState.monthlyIncome * 0.25),
                    used: cats.future?.used || 0,
                    percentage: cats.future?.percentage || 25
                },
                freedom: {
                    allocated: cats.freedom?.allocated || Math.round(appState.monthlyIncome * 0.15),
                    used: cats.freedom?.used || 0,
                    percentage: cats.freedom?.percentage || 15
                }
            };

            // Set allocation state for v4.3
            allocationState = {
                foundation: cats.foundation?.percentage || 60,
                future: cats.future?.percentage || 25,
                freedom: cats.freedom?.percentage || 15,
                originalAllocations: {
                    foundation: 60,
                    future: 25,
                    freedom: 15
        ...
        // Calculate daily flow using enhanced calculateDailyFlow function
        const newDailyFlow = calculateDailyFlow(appState.categories);
        ...
        if (savedData.achievements) {
            appState.achievements = savedData.achievements;
        }

        // Set user profile
        if (savedData.userProfile) {
            appState.userProfile = savedData.userProfile.savingsProfile || 'starting';
        }

        console.log('✅ v4.3 data processing completed:', {
            income: appState.monthlyIncome,
            periodType: appState.periodConfig.type,
            periodLength: appState.periodConfig.length,
        ...
        console.error('❌ Error processing v4.3 data:', error);
        return false;
    }
}
...
function updateCategoryCards() {
    ...
    // Update daily flow in Spend tab using enhanced calculateDailyFlow function
    const dailyFlow = calculateDailyFlow(appState.categories);
...
function updateImpactPreview() {
    ...
    // Create temporary categories state to calculate daily flow with current allocations
    const tempCategories = {
        freedom: {
            allocated: Math.round((allocationState.freedom / 100) * income),
            used: appState.categories?.freedom?.used || 0
        }
    };
    const currentDailyFlow = calculateDailyFlow(tempCategories);
...
document.addEventListener('DOMContentLoaded', function () {
    console.log('🎯 FLOW BUDGETING v4.3.0 - FLEXIBLE PERIOD SYSTEM');
    console.log('📅 Phase 1: FLEXIBLE PERIODS & GIGI ECONOMY SUPPORT');
    ...
    if (!appState.categories || !appState.categories.freedom) {
        ...
        // Initialize default period config
        appState.periodConfig = {
            type: 'monthly',
            length: 30,
            startDate: new Date().toISOString().split('T')[0],
            incomeAmount: 3200,
            lastReset: null,
            nextReset: calculateNextReset(new Date().toISOString().split('T')[0], 'monthly')
        };
    }

    // V4.3: Try to restore data with period system support
    ...
    if (dataRestored && appState.onboardingComplete) {
        ...
        console.log('✅ Returning v4.3 user detected, skipping onboarding');
        const overlay = document.getElementById('onboardingOverlay');
        if (overlay) {
            overlay.style.display = 'none';
        }

        // V4.3: Period system check and validation
        if (appState.periodConfig) {
            console.log('🔄 Checking period system status:', {
                type: appState.periodConfig.type,
                nextReset: appState.periodConfig.nextReset,
                dailyFlow: appState.dailyFlow
            });
            
            // Check for period reset on app load
            checkAndHandlePeriodReset();
        }
        ...
        // V4.3: Check for fresh onboarding completion before initializing defaults
        const v43OnboardingDetected = checkForV43OnboardingData();
        if (v43OnboardingDetected) {
            console.log('✅ v4.3 onboarding data processed successfully');
            // Initialization done in checkForV43OnboardingData
            return;
        }
        
        // New user or data restore failed - initialize with v4.3 defaults
        console.log('ℹ️ New user or incomplete data, initializing with v4.3 defaults');
    ...
    // **DUAL DISPLAY SYSTEM: Initialize daily flow for new day**
    assignDailyFlow();
...
function updatePreview() {
    ...
    const tempCategories = {
        freedom: {
            allocated: freedomAmount,
            used: appState.categories?.freedom?.used || 0
        }
    };
    const dailyFlow = calculateDailyFlow(tempCategories);
    ...
    const originalFreedomAmount = Math.round((income * allocationState.originalAllocations.freedom / 100));
    const originalTempCategories = {
        freedom: {
            allocated: originalFreedomAmount,
            used: appState.categories?.freedom?.used || 0
        }
    };
    const originalDaily = calculateDailyFlow(originalTempCategories);
...
function runMathematicalValidationTest() {
    ...
    const expectedRemainingBudgetFlow = Math.round(remainingBudget / daysRemaining); // $1 precision
...
function assignDailyFlow() {
    ...
    try {
        // **FIXED: Use same calculation logic as calculateDailyFlow for mathematical consistency**
        ...
        // **PERIOD-AWARE CALCULATION: Use same logic as calculateDailyFlow**
        let daysToUse;
        if (appState?.periodConfig?.length && appState.periodConfig.length > 0) {
            // Period-based: Use full period length for fixed daily amount
            daysToUse = appState.periodConfig.length;
            console.log(`📊 Using period-based calculation: ${daysToUse} days`);
        } else {
            // Monthly fallback: Use days remaining in month
            const currentDay = new Date().getDate();
            const daysInMonth = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate();
            daysToUse = Math.max(daysInMonth - currentDay + 1, 1);
            console.log(`📅 Using monthly calculation: ${daysToUse} days remaining`);
        }

        // Calculate daily flow using consistent logic
        const rawDailyFlow = remaining / daysToUse;
        ...
        console.log('✨ Today\'s Flow assigned:', {
            ...
            remaining: remaining,
            daysToUse: daysToUse,
            calculation: `$${remaining} ÷ ${daysToUse} = $${rawDailyFlow.toFixed(2)} → $${todaysFlow}`,
...
function testDay36Implementation() {
    ...
    try {
        ...
        const spendUsed = appState.categories?.freedom?.used || appState.categories?.spend?.used || 0;
        const remainingBudget = spendAllocation - spendUsed;
        const expectedDailyFlow = Math.round(remainingBudget / daysRemaining); // $1 precision
...
// ===== NEW DEBUG FUNCTION: COMPREHENSIVE DAILY FLOW ANALYSIS =====
function debugDailyFlowDetailed() {
    console.log('🔍 COMPREHENSIVE DAILY FLOW CALCULATION ANALYSIS');
    console.log('=====================================================');
    ...
    // Calculate days remaining based on period configuration
    let daysRemaining;
    let periodDaysElapsed = 0;
    let periodDaysRemaining = 0;
    let usingPeriodCalculation = false;
    
    if (appState?.periodConfig?.length && appState.periodConfig.length > 0) {
        // Period-based calculation
        usingPeriodCalculation = true;
        const periodLength = appState.periodConfig.length;
        ...
        if (appState.periodConfig.startDate) {
            ...
            periodDaysElapsed = daysDiff % periodLength; // Days elapsed in current period cycle
            // Days remaining includes today, so it's periodLength - periodDaysElapsed  
            periodDaysRemaining = Math.max(periodLength - periodDaysElapsed, 1);
            
            console.log(`   🔍 PERIOD DATE CALCULATION DETAILS:`);
            console.log(`      Period Start Date: ${appState.periodConfig.startDate}`);
            console.log(`      Today's Date: ${today.toISOString().split('T')[0]}`);
            console.log(`      Start Date (normalized): ${startDate.toISOString().split('T')[0]}`);
            console.log(`      Raw Days Diff: ${daysDiff}`);
            console.log(`      Days Diff Math: Math.floor((${today.toISOString().split('T')[0]} - ${startDate.toISOString().split('T')[0]}) / (1000*60*60*24))`);
            console.log(`      Period Days Elapsed: ${periodDaysElapsed} (${daysDiff} % ${periodLength})`);
            console.log(`      Period Days Remaining: ${periodDaysRemaining} (${periodLength} - ${periodDaysElapsed})`);
        } else {
            // Fallback: assume we're at the beginning of a period
            periodDaysElapsed = 0;
            periodDaysRemaining = periodLength;
        }
        
        daysRemaining = periodDaysRemaining;
    } else {
        // Monthly-based calculation (fallback)
        daysRemaining = Math.max(daysInMonth - currentDay + 1, 1);
    }
    ...
    if (usingPeriodCalculation) {
        console.log(`   Period Length: ${appState.periodConfig.length} days`);
        console.log(`   Period Days Elapsed: ${periodDaysElapsed}`);
        console.log(`   Period Days Remaining: ${periodDaysRemaining}`);
        console.log(`   Using Period-Based Days Remaining: ${daysRemaining}`);
    } else {
        console.log(`   Monthly Days Remaining: ${daysRemaining}`);
    }
    console.log('');
    
    // App State Analysis
    console.log('🏦 APP STATE ANALYSIS:');
    console.log(`   Monthly Income: $${appState.monthlyIncome || 'Not Set'}`);
    console.log(`   User Profile: ${appState.userProfile || 'Not Set'}`);
    console.log(`   Onboarding Complete: ${appState.onboardingComplete}`);
    console.log(`   Current Period: ${appState.currentPeriod || 'Not Set'}`);
    console.log('');
    
    // Period Configuration Analysis
    console.log('📊 PERIOD CONFIGURATION ANALYSIS:');
    if (appState.periodConfig && appState.periodConfig.length) {
        console.log(`   Period Type: ${appState.periodConfig.type}`);
        console.log(`   Period Length: ${appState.periodConfig.length} days`);
        console.log(`   Start Date: ${appState.periodConfig.startDate || 'Not Set'}`);
        console.log(`   Income Amount: $${appState.periodConfig.incomeAmount || 'Not Set'}`);
        console.log(`   Last Reset: ${appState.periodConfig.lastReset || 'Never'}`);
        console.log(`   Next Reset: ${appState.periodConfig.nextReset || 'Not Scheduled'}`);
    } else {
        console.log('   No period configuration found (using monthly default)');
    }
    console.log('');
    ...
    ['foundation', 'future', 'freedom', 'secure', 'save', 'spend'].forEach(categoryName => {
        const category = categories[categoryName];
        if (category) {
            console.log(`   ${categoryName.toUpperCase()}:`);
            console.log(`      Allocated: $${category.allocated || 0}`);
            console.log(`      Used: $${category.used || 0}`);
            console.log(`      Remaining: $${(category.allocated || 0) - (category.used || 0)}`);
            console.log(`      Percentage: ${category.percentage || 0}%`);
            console.log('');
        }
    });
    
    // Focus on Freedom/Spend Category (main calculation category)
    const freedomCategory = categories.freedom || categories.spend;
    console.log('🎯 DAILY FLOW CALCULATION CATEGORY:');
    if (freedomCategory) {
        console.log(`   Category Name: ${categories.freedom ? 'freedom' : 'spend'}`);
        console.log(`   Allocated: $${freedomCategory.allocated}`);
        console.log(`   Used: $${freedomCategory.used}`);
        console.log(`   Remaining: $${freedomCategory.allocated - freedomCategory.used}`);
        console.log(`   Usage %: ${((freedomCategory.used / freedomCategory.allocated) * 100).toFixed(1)}%`);
    } else {
        console.log('   ERROR: No freedom or spend category found!');
    }
    console.log('');
    
    // Manual Daily Flow Calculation Step by Step
    console.log('🧮 STEP-BY-STEP DAILY FLOW CALCULATION:');
    
    if (freedomCategory) {
        const allocated = freedomCategory.allocated;
        const used = freedomCategory.used;
        const remaining = allocated - used;
        
        console.log(`   Step 1 - Allocated Amount: $${allocated}`);
        console.log(`   Step 2 - Amount Used: $${used}`);
        console.log(`   Step 3 - Remaining Budget: $${allocated} - $${used} = $${remaining}`);
        
        // Check for period-based vs monthly calculation
        if (appState?.periodConfig?.length && appState.periodConfig.length > 0) {
            const periodLength = appState.periodConfig.length;
            const rawDailyFlow = remaining / periodLength;
            const roundedDailyFlow = Math.max(0, Math.round(rawDailyFlow));
            
            console.log(`   Step 4 - PERIOD-BASED CALCULATION:`);
            console.log(`      Period Length: ${periodLength} days`);
            console.log(`      Raw Daily Flow: $${remaining} ÷ ${periodLength} = $${rawDailyFlow.toFixed(2)}`);
            console.log(`      Rounded Daily Flow: Math.round($${rawDailyFlow.toFixed(2)}) = $${roundedDailyFlow}`);
        } else {
            const rawDailyFlow = remaining / daysRemaining;
            const roundedDailyFlow = Math.max(0, Math.round(rawDailyFlow));
            
            console.log(`   Step 4 - MONTHLY-BASED CALCULATION:`);
            console.log(`      Days Remaining: ${daysRemaining} days`);
            console.log(`      Raw Daily Flow: $${remaining} ÷ ${daysRemaining} = $${rawDailyFlow.toFixed(2)}`);
            console.log(`      Rounded Daily Flow: Math.round($${rawDailyFlow.toFixed(2)}) = $${roundedDailyFlow}`);
        }
    }
    console.log('');
    ...
    // Method 1: Current calculateDailyFlow function
    const currentResult = calculateDailyFlow(appState.categories);
    console.log(`   Current calculateDailyFlow(): $${currentResult}`);
    
    // Method 2: Unified calculation
    if (typeof calculateDailyFlowUnified === 'function' && freedomCategory) {
        const unifiedResult = calculateDailyFlowUnified({
            spendAllocated: freedomCategory.allocated,
            spendUsed: freedomCategory.used,
            currentDay: currentDay,
            useRemainingDays: true,
            forceFullAllocation: false
        });
        console.log(`   calculateDailyFlowUnified(): $${unifiedResult}`);
    }
    
    // Method 3: Onboarding calculation
    if (typeof calculateDailyFlowOnboarding === 'function') {
        const onboardingResult = calculateDailyFlowOnboarding(appState.monthlyIncome || 3200, 0.05);
        console.log(`   calculateDailyFlowOnboarding(): $${onboardingResult}`);
    }
    
    // Method 4: Manual verification
    if (freedomCategory) {
        const manualRemaining = freedomCategory.allocated - freedomCategory.used;
        const manualDaily = Math.round(manualRemaining / daysRemaining);
        console.log(`   Manual Verification: $${manualDaily}`);
    }
    console.log('');
    
    // Transaction History Impact
    console.log('💳 RECENT TRANSACTION IMPACT:');
    const recentTransactions = appState.transactions
    ...
    if (recentTransactions.length > 0) {
        ...
        recentTransactions.forEach((t, index) => {
            ...
            console.log(`      ${index + 1}. $${t.amount} - ${t.description} (${date})`);
        });
        
        const todayTransactions = appState.transactions
            .filter(t => {
                const transactionDate = new Date(t.timestamp).toDateString();
                const today = new Date().toDateString();
                return transactionDate === today && (t.category === 'freedom' || t.category === 'spend');
            });
        
        const todaySpent = todayTransactions.reduce((sum, t) => sum + t.amount, 0);
        console.log(`   Today's Spending: $${todaySpent} (${todayTransactions.length} transactions)`);
    } else {
        console.log('   No recent transactions found');
    }
    console.log('');
    
    // Dual Display System Analysis
    console.log('📱 DUAL DISPLAY SYSTEM ANALYSIS:');
    console.log(`   dailyFlow (calculated): $${appState.dailyFlow || 'Not Set'}`);
    console.log(`   dailyFlowFixed (assigned): $${appState.dailyFlowFixed || 'Not Set'}`);
    console.log(`   todayFlowed: $${appState.todayFlowed || 0}`);
    console.log(`   lastDayStart: ${appState.lastDayStart ? new Date(appState.lastDayStart).toLocaleString() : 'Not Set'}`);
    
    // **CORRECTED DUAL DISPLAY CALCULATION**
    if (appState.todayFlowed !== undefined && appState.dailyFlowFixed !== undefined) {
        const dailyFlowFixed = appState.dailyFlowFixed || currentResult; // Fallback to calculated if not set
        const remainingToday = dailyFlowFixed - appState.todayFlowed;
        console.log(`   DUAL DISPLAY MAIN: $${dailyFlowFixed} - $${appState.todayFlowed} = $${remainingToday}`);
        console.log(`   DUAL DISPLAY TOMORROW: $${currentResult} (recalculated amount)`);
        
        if (appState.dailyFlowFixed === null || appState.dailyFlowFixed === undefined) {
            console.log(`   ⚠️  WARNING: dailyFlowFixed not set - assignDailyFlow() may not have run`);
        }
    }
    console.log('');
    ...
    if (!freedomCategory) {
        console.log('   ❌ CRITICAL: No freedom or spend category found - check category structure');
    }
    
    if (appState.dailyFlowFixed === null || appState.dailyFlowFixed === undefined) {
        console.log('   ⚠️  DUAL DISPLAY: dailyFlowFixed not initialized - call assignDailyFlow()');
    } else {
        console.log('   ✅ DUAL DISPLAY: dailyFlowFixed properly initialized');
    }
    
    if (appState.periodConfig && appState.periodConfig.length && appState.periodConfig.length > 0) {
        console.log('   📊 Using period-based calculation - verify period configuration is correct');
    } else {
        console.log('   📅 Using monthly-based calculation - this is the default behavior');
    }
    
    if (currentResult === 0) {
        console.log('   ⚠️  Daily flow is $0 - check if budget is fully used or remaining days calculation');
    }
    
    if (appState.transactions.length === 0) {
        console.log('   ℹ️  No transactions found - calculations based on pure allocation');
    }
    
    console.log('=====================================================');
    
    // Return summary object for further debugging
    return {
        dateInfo: { currentDay, daysInMonth, daysRemaining },
        categoryInfo: freedomCategory,
        calculation: {
            current: currentResult,
            manual: freedomCategory ? Math.round((freedomCategory.allocated - freedomCategory.used) / daysRemaining) : null
        },
        periodBased: !!(appState?.periodConfig?.length),
        todaySpent: appState.todayFlowed || 0
    };
}

// Make the function available globally for easy console access
window.debugDailyFlowDetailed = debugDailyFlowDetailed;
...
function testMathematicalAccuracy() {
    ...
    try {
        ...
        const expectedDailyFlow = Math.round(rawDailyFlow); // $1 precision
...
function updateIncomePreview() {
    ...
    // Calculate new allocations using enhanced calculateDailyFlow function
    ...
    // Create temporary categories for new income to calculate daily flow
    const newTempCategories = {
        freedom: {
            allocated: newFreedom,
            used: appState.categories?.freedom?.used || 0
        }
    };
    const newDailyFlow = calculateDailyFlow(newTempCategories);

    // Update preview
    const currentTempCategories = {
        freedom: {
            allocated: Math.round(appState.monthlyIncome * allocationState.freedom / 100),
            used: appState.categories?.freedom?.used || 0
        }
    };
    const currentDailyFlow = calculateDailyFlow(currentTempCategories);
...
function runDay38Phase2CodeReview() {
    ...
    try {
        ...
        const expectedDailyFlow = Math.round(spendRemaining / daysRemaining); // $1 precision

        const dailyFlowCorrect = Math.abs(dailyFlowCalculation - expectedDailyFlow) < 1; // Allow some variance
...
function updateIncomePreview(newIncome) {
    try {
        ...
        if (profileData) {
            // Calculate daily flow using enhanced function
            const freedomAmount = Math.round((newIncome * profileData.spend_percentage / 100));
            const tempCategories = {
                freedom: {
                    allocated: freedomAmount,
                    used: 0 // For preview, assume no spending yet
                }
            };
            const dailyFlow = calculateDailyFlow(tempCategories);
...
// ===== V4.3: FLEXIBLE PERIOD SYSTEM FUNCTIONS =====

/**
 * Initialize period configuration from onboarding data
 */
function initializePeriodConfig(onboardingData) {
    if (onboardingData && onboardingData.periodConfig) {
        appState.periodConfig = {
            type: onboardingData.periodConfig.type || 'monthly',
            length: onboardingData.periodConfig.length || 30,
            startDate: onboardingData.periodConfig.startDate || new Date().toISOString().split('T')[0],
            incomeAmount: onboardingData.periodConfig.incomeAmount || onboardingData.monthlyIncome || 3200,
            lastReset: null,
            nextReset: calculateNextReset(onboardingData.periodConfig.startDate, onboardingData.periodConfig.type)
        };
        
        console.log('✅ Period config initialized:', appState.periodConfig);
        return true;
    }
    ...
    appState.periodConfig = {
        ...
        length: 30,
        startDate: new Date().toISOString().split('T')[0],
        incomeAmount: appState.monthlyIncome || 3200,
        lastReset: null,
        nextReset: null
    };
...
/**
 * Calculate next period reset date based on start date and period type
 */
function calculateNextReset(startDate, periodType) {
    ...
    let periodLength;
    switch(periodType) {
        case 'weekly': periodLength = 7; break;
        case 'biweekly': periodLength = 14; break;
        case 'monthly': periodLength = 30; break;
        default: periodLength = 30;
    }
    
    // Calculate how many periods have passed since start date
    const daysSinceStart = Math.floor((now - start) / (1000 * 60 * 60 * 24));
    const periodsCompleted = Math.floor(daysSinceStart / periodLength);
    
    // Calculate next reset date
    const nextReset = new Date(start);
    nextReset.setDate(start.getDate() + ((periodsCompleted + 1) * periodLength));
    
    return nextReset.toISOString().split('T')[0];
}

/**
 * Check if period reset is needed and perform reset
 */
function checkAndHandlePeriodReset() {
    if (!appState.periodConfig.nextReset) return false;
    
    const now = new Date();
    const resetDate = new Date(appState.periodConfig.nextReset);
    
    if (now >= resetDate) {
        // Perform period reset
        performPeriodReset();
        return true;
    }
...
/**
 * Perform period reset - clear freedom category usage and calculate new dates
 */
function performPeriodReset() {
    console.log('🔄 Performing period reset...');
    
    // Reset freedom category usage
    if (appState.categories && appState.categories.freedom) {
        appState.categories.freedom.used = 0;
    }
    
    // Update period tracking
    appState.periodConfig.lastReset = new Date().toISOString().split('T')[0];
    appState.periodConfig.nextReset = calculateNextReset(
        appState.periodConfig.lastReset, 
        appState.periodConfig.type
    );
    
    // Recalculate daily flow using enhanced calculateDailyFlow function
    const newDailyFlow = calculateDailyFlow(appState.categories);
    appState.dailyFlow = newDailyFlow;
    appState.dailyFlowAmount = newDailyFlow;
    ...
    showToast(`🔄 New ${appState.periodConfig.type} period started! Your freedom amount has reset.`, 'success');
    
    console.log('✅ Period reset completed:', {
        lastReset: appState.periodConfig.lastReset,
        nextReset: appState.periodConfig.nextReset,
        newDailyFlow
    });
...
/**
 * Migrate v4.2 data to v4.3 format
 */
function migrateV42ToV43(v42Data) {
    console.log('🔄 Migrating v4.2 data to v4.3...');
    ...
    const migratedData = {
        ...
        // Migrate basic user profile
        userProfile: {
            userName: v42Data.userProfile?.userName || 'Flow User',
            monthlyIncome: v42Data.userProfile?.monthlyIncome || 3200,
            savingsProfile: v42Data.userProfile?.savingsProfile || 'starting',
            primaryGoal: 'imported-from-v42',
            selectedHabits: ['daily-tracking', 'morning-checkin'],
            onboardingCompleted: true,
            setupCompleted: true,
            lastUpdated: Date.now()
        },
        
        // V4.3 period configuration (default to monthly for migrated users)
        periodConfig: {
            type: 'monthly',
            length: 30,
            startDate: new Date().toISOString().split('T')[0],
            incomeAmount: v42Data.userProfile?.monthlyIncome || 3200,
            lastReset: null,
            nextReset: calculateNextReset(new Date().toISOString().split('T')[0], 'monthly')
        },
        
        // Migrate and update allocations to v4.3 Flow Method
        categories: {
            foundation: { 
                percentage: 60,
                allocated: Math.round((v42Data.userProfile?.monthlyIncome || 3200) * 0.60),
                used: v42Data.budgetState?.categories?.foundation?.used || 0
            },
            future: { 
                percentage: 25,
                allocated: Math.round((v42Data.userProfile?.monthlyIncome || 3200) * 0.25),
                used: v42Data.budgetState?.categories?.future?.used || 0
            },
            freedom: {
                percentage: 15,
                allocated: Math.round((v42Data.userProfile?.monthlyIncome || 3200) * 0.15),
                used: v42Data.budgetState?.categories?.freedom?.used || 0
            }
        },
        
        // Migrate existing transactions
        transactions: v42Data.transactions || [],
        
        // Migrate period history
        periodHistory: v42Data.periodHistory || [],
        
        // Migrate achievements
        achievements: v42Data.achievements || {
            badges: [],
            currentXP: 0,
            currentLevel: 1,
            ...
            avatar: "🌱",
            streaks: {
                dailyFlow: { current: 0, longest: 0 },
                mindfulSpending: { current: 0, longest: 0 },
                savingsContribution: { current: 0, longest: 0 }
            }
        },
        
        // Calculate daily flow with new period system
        dailyFlow: Math.round((Math.round((v42Data.userProfile?.monthlyIncome || 3200) * 0.15)) / 30),
        ...
        migrationInfo: {
            ...
            toVersion: '4.3',
            migratedAt: Date.now(),
            migratedBy: 'v4.3-automatic-migration'
        }
    };
...
/**
 * Load v4.3 data with fallback to v4.2 migration
 */
function loadV43DataWithMigration() {
    try {
        // First, try to load v4.3 data
        const v43Data = localStorage.getItem('flowAppData_v4_3');
        if (v43Data) {
            const parsed = JSON.parse(v43Data);
            console.log('✅ Loaded existing v4.3 data');
            return parsed;
        }
        
        // Fallback: Check for v4.2 data to migrate
        const v42Data = localStorage.getItem('flowBudgeting_v3'); // v4.2 used this key
        if (v42Data) {
            const parsedV42 = JSON.parse(v42Data);
            console.log('📦 Found v4.2 data, migrating to v4.3...');
            
            const migratedData = migrateV42ToV43(parsedV42);
            
            // Save migrated data to v4.3 namespace
            localStorage.setItem('flowAppData_v4_3', JSON.stringify(migratedData));
            
            showToast('✨ Welcome to Flow v4.3! Your data has been upgraded with flexible periods.', 'success');
            
            return migratedData;
        }
        
        // No existing data found
        console.log('ℹ️ No existing data found - new user setup required');
        return null;
        
    } catch (error) {
        console.error('❌ Error loading v4.3 data:', error);
        return null;
    }
}

// ===== END V4.3 PERIOD SYSTEM FUNCTIONS =====